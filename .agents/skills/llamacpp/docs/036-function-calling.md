...
source: git
fetched_at: 2026-04-28T09:49:28.142398047-03:00
rendered_js: false
word_count: 1705
summary: This document explains the implementation of OpenAI-style function calling within the llama.cpp framework, detailing support for native and generic tool-calling formats across various models.
tags:
    - llama-cpp
    - function-calling
    - tool-use
...
[chat.h](../common/chat.h) ([PR #9639](https://github.com/ggml-org/llama.cpp/pull/9639)) adds [OpenAI-style function calling](https://platform.openai.com/docs/guides/function-calling) support. Used in `llama-server` with `--jinja` flag.

## Universal Support: Native & Generic Handlers

Function calling supported for all models ([PR #9639](https://github.com/ggml-org/llama.cpp/pull/9639)):

**Native formats**:
- Llama 3.1 / 3.3 (builtin tools: `wolfram_alpha`, `web_search`/`brave_search`, `code_interpreter`), Llama 3.2
- Functionary v3.1 / v3.2, Hermes 2/3, Qwen 2.5, Qwen 2.5 Coder
- Mistral Nemo, Firefunction v2, Command R7B
- DeepSeek R1 (WIP)

**Generic format**: When template not recognized by native handlers (`Chat format: Generic` in logs). Use `--chat-template-file` to override. May consume more tokens than native format.

**Parallel tool calling**: Supported on some models, disabled by default. Enable via `"parallel_tool_calls": true` in completion endpoint payload.
...
## Usage — Tool-Aware Jinja Template Required

Start a server with a tools-enabled template. Verify via `chat_template` or `chat_template_tool_use` in `http://localhost:8080/props`.
...
### Native with Template Override

DeepSeek R1 works best with our override (official template is buggy):
...
### Native Requiring Right Template
...
Get official templates from HuggingFace with [scripts/get_chat_template.py](../scripts/get_chat_template.py) (examples in [models/templates/README.md](../models/templates/README.md)).

> [!tip]
> If no official `tool_use` template exists, try `--chat-template chatml` for a default, or write your own (e.g. [llama-cpp-deepseek-r1.jinja](../models/templates/llama-cpp-deepseek-r1.jinja) for DeepSeek R1 distills).

> [!warning]
> Extreme KV quantizations (e.g. `-ctk q4_0`) can substantially degrade tool calling performance.

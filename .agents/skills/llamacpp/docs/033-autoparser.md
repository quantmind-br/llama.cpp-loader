---
title: Autoparser
url: https://github.com/ggml-org/llama.cpp/blob/master/docs/autoparser.md
source: git
fetched_at: 2026-04-28T09:49:08.915774398-03:00
rendered_js: false
optimized: true
summary: Differential compositional system that auto-analyzes chat templates to construct PEG parsers for reasoning, content, and tool call extraction.
tags:
    - chat-templates
    - peg-parser
    - tool-calling
...
Auto-analyzes chat templates → PEG parsers for content, reasoning, and tool calls.

## Overview

Pure differential, compositional approach (inspired by `git diff`) to analyze chat templates:

- **Minimize Hardcoded Patterns**: all markers extracted via template comparison; only heuristic = JSON detection (`JSON_NATIVE` vs tag-based)
- **Compositional Architecture**: separate analyzer structs for reasoning, content, tools — each owns analysis + parser construction

**Two-step pipeline**:
1. `autoparser::autoparser tmpl_analysis(tmpl)` — differential comparisons → analysis structs
2. `autoparser::peg_generator::generate_parser(tmpl, generation_params, tmpl_analysis)` → PEG parser + optional GBNF

## Data Structures

All in [common/chat-auto-parser.h](common/chat-auto-parser.h).

### `autoparser` (top-level)

[L367-388](common/chat-auto-parser.h#L367-L388) — aggregates `jinja_caps`, `reasoning`, `content`, `tools`, plus `preserved_tokens` (union of all non-empty markers).

### `analyze_reasoning`

[L254-274](common/chat-auto-parser.h#L254-L274) — `mode` enum, `start` (e.g. `<think>`), `end` (e.g. `</think>`) markers.

### `analyze_content`

[L280-295](common/chat-auto-parser.h#L280-L295) — `mode` enum, `start`/`end` markers, `requires_nonnull_content` flag.

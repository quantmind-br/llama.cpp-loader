---
title: MobileVLM
url: https://github.com/ggml-org/llama.cpp/blob/master/docs/multimodal/MobileVLM.md
source: git
fetched_at: 2026-04-28T09:49:31.766386182-03:00
rendered_js: false
word_count: 318
summary: This document provides instructions for converting, building, and running MobileVLM models on Android devices using the llama-mtmd-cli implementation.
tags:
    - mobilevlm
    - android
    - model-conversion
...
Supported variants: [MobileVLM-1.7B](https://huggingface.co/mtgv/MobileVLM-1.7B) and [MobileVLM_V2-1.7B](https://huggingface.co/mtgv/MobileVLM_V2-1.7B). See [Meituan-AutoML/MobileVLM](https://github.com/Meituan-AutoML/MobileVLM) for details.

Implementation is based on llava and compatible with both llava and MobileVLM. Inference is identical for **MobileVLM** and **MobileVLM_V2**; conversion differs slightly. Steps below use MobileVLM-1.7B as the example, with V2 differences noted.
...
2. Split LLaVA model to LLaMA and multimodal projector constituents:

```sh
python ./tools/mtmd/llava_surgery.py -m path/to/MobileVLM-1.7B
```

3. Convert the image encoder to GGUF. Use `--projector-type ldp` for **V1** or `--projector-type ldpv2` for **V2**:
...
## Results on Android (Snapdragon 888)
...
## Results on Android (Snapdragon 778G)
...
## Results on Intel Core i7-10750H (Ubuntu 22.04)
...
## Results on Intel Core Ultra7 115H (Windows 11)
...
- [x] Support non-CPU backend for new operators (`depthwise`, `hardswish`, `hardsigmoid`)
- [ ] Optimize LDP projector performance:
    - Optimize structure definition to avoid unnecessary memory rearrangements and reduce `ggml_permute_cpy` usage
    - Optimize operator implementation (ARM CPU/NVIDIA GPU): depthwise conv, hardswish, hardsigmoid
- [x] Run MobileVLM on Jetson Orin
- [ ] Support more model variants (e.g. `MobileVLM-3B`)
...
#mobilevlm #android #model-conversion #llava
